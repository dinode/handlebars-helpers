...
describe('date', function () {
  describe('isodate', function () {
    ...
    it("should return today's date", function () {
      ...
      assert.equal(fn(), moment().startOf('day').toISOString());
    });

    it('should add the days', function () {
      var fn = hbs.compile('{{isodate "add" 2}}');

      assert.equal(fn(), moment().add(2, 'days').toISOString());
    });

    it('should subtract the days', function () {
      var fn = hbs.compile('{{isodate "subtract" 2}}');

      assert.equal(fn(), moment().subtract(2, 'days').toISOString());
    });

    it('should format date', function () {
      var fn = hbs.compile(
        "{{formatdate '2022-02-01T00:00:00.000+11:00' 'YYYY-MM-DD'}}"
      );
    ...
    it('should format date when combined with isodate', function () {
      ...
      assert.equal(fn(), moment().startOf('day').format('YYYY-MM-DD'));
    });
  });
});
