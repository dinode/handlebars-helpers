...
/**
 * Flatten an array by the given depth
 *
 * ```handlebars
 * <!-- array: ['b', 'a', ['c']] -->
 * {{flat array 1}}
 ...
 * @param {Number} `depth` The depth of arrays to flatten.
 * @api public
 */

 helpers.flat = function (array, depth) {
  if (!Array.isArray(array)) return "";
...
/**
 * Map over the given array of objects and extract _id properties
 * and return as a JSON array string
 *
 * ```handlebars
 * // {{pluckIds items}}
 * <!-- results in: '["aa", "bb", "cc"]' -->
...
helpers.pluckIds = function (arr) {
  const json = JSON.stringify(arr ? map(castArray(arr), "_id") : []);
